package Lab0;
//Java program to implement Round Robin with arrival time
import java.util.*;
public class RoundRobinwitharrival {
	public static void findWaitingTime(int process[],int wt_time[],int n ,int brusttime[],int quantum,
			int completion_time[],int arrival_time[])
		{
	...
	public static void findTurnAroundTime(int process[] ,int wt_time[],int n,int brusttime[],int tat_time[],
			int completion_time[],int arrival_time[])
	{
		for(int i=0;i<n;i++){
	...
	RoundRobinwitharrival object = new RoundRobinwitharrival();
	Scanner scan = new Scanner(System.in);
	
	int quantum = 2;
	int arrival_time[] = new int[]{0,1,2,3};
	int process[] = new int[]{1,2,3,4};
	int brusttime[] = new int[]{7,5,3,1};
